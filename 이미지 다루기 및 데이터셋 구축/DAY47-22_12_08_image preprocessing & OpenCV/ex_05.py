...
def drawhoughLinesOnImage(image, hooughLine): # 공유 이미지 중 '공식.png ,공식2.png' 참조   
...
def draw_circle(img, circle):
    for co, i, in enumerate(circle[0, :], start=1):
        print(i[0], i[1])
        cv2.circle(img, (int(i[0]), int(i[1])), int(i[2]), (255, 0, 255), 3) # i 앞에 int는 꼭 붙여야됨!
...
# 3. 가우시안 블러 적용
blurredImage = cv2.GaussianBlur(gray_image, (5, 5), 0) # (이미지, 필터사이즈, )
...
houghImage = np.zeros_like(image)# np.zeros와의 차이는 0이 가득한 배열이냐 이미지 사이즈만큼의 0을 채운 배열이냐의 차이
