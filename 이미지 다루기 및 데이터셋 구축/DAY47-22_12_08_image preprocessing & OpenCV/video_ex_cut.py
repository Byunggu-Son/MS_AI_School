# 동영상 파일 프레임 나눠서 저장하기
import cv2
import os
video_file_path = "./video01.mp4"

# 동영상 캡처 객체 생성
cap = cv2.VideoCapture(video_file_path)

fps = cap.get(cv2.CAP_PROP_FPS)
print("fps >>" , fps)
count = 0 # 몇 장의 이미지가 저장됐는지를 보기 위함
if cap.isOpened(): 
    while True:
        ret, frame = cap.read() 
        if ret: # 읽어왔다면 바로 밑 if문 실행
            if (int(cap.get(1)) % fps == 0): # fps값을 사용하여 1초마다 추출
                os.makedirs('./frame_image_save', exist_ok = True) # 폴더가 있으면 넘어가고 없으면 생성. 예외처리 대신하는게 exist_ok(있으면 오류말고 넘어가라)
                cv2.imwrite('./frame_image_save/' + 'frame%d.jpg' % count, frame)
                print('save frame number >>', str(int(cap.get(1))))
                count +=1
        else :
            break
else:
    print('비디오 파일 읽기 실패')
cap.release()
cv2.destroyAllWindows()

'''
결과
fps >> 25.0
save frame number >> 25
save frame number >> 50
save frame number >> 75
save frame number >> 100
save frame number >> 125
save frame number >> 150
save frame number >> 175
save frame number >> 200
save frame number >> 225
save frame number >> 250
save frame number >> 275
save frame number >> 300

frame_image_save에 프레임 단위로 자른 사진들이 저장.
'''