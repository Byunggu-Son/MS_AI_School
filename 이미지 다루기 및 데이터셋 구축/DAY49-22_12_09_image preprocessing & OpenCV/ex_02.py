# Selenium을 이용한 데이터셋 확보 및 데이터 셋 생성
import PIL
...
def main():
    option = Options()
    # 포털검색막힐 때 쓰기. useragent에 들어가서 본인꺼 쓰면 됨.
    option.add_argument('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36') 
    ...
    for search in search_list:
        ...
        for idx, img_src in enumerate(img_srcs):
            ...
            except PIL.UnidentifiedImageError: # 에러 처리
                print(base64_image)
            except AttributeError:
                print(base64_image)
                continue
            if int(N) == idx: # string으로 가져오기에 int로 형변환
                break
